import { lookupSymbol } from "./symbols.ts";
import { commands } from "./commands.ts";
import { modifiers } from "./modifier.ts";
import { subTable, supTable } from "./supsub.ts";
...
export class CommandContext extends Context {
  public consume(buf: SourceBuf): ConsumeResult {
    ...
    if (s != null && originalCursorPosition !== inputPosition + text.length) {
      ...
      if (
        (value[buf.inputPosition + movelen] === " " ||
          value[buf.inputPosition + movelen] === "\u00a0") &&
        buf.inputPosition + movelen + 1 === originalCursorPosition
      ) {
    ...
    if (com != null) {
      ...
      if (next === "{") {
        ...
        if (blockComplete) {
          ...
          if (inputPosition < originalCursorPosition) {
            // コマンドと{の分が消える
            buf.cursorPosition -= Math.min(
              originalCursorPosition - inputPosition,
              text.length + 1,
            );
          }
